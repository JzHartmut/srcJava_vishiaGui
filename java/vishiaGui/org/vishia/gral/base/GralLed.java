...
public class GralLed extends GralWidget
{
  ...
  { super(currPos, sPosName, 'D');
    ...
    this.colorBorderSelectable[2] = GralColor.getColor("ma");  //faulty showing color
    ...
    this.colorInnerSelectable[2] = GralColor.getColor("ma");   //faulty showing color
  ...
  public void setColor(GralColor colorBorder, GralColor colorInner){
    this.dyda.lineColor = colorBorder;
    this.dyda.backColor = colorInner;
  ...
  @Override public void setValue(float value){
    int ival = (int)value;
    if(ival < 0){ ival = 0; }
    else if(ival >= this.colorInnerSelectable.length){ ival = this.colorInnerSelectable.length -1;}
    this.dyda.lineColor = this.dyda.backColor = this.colorInnerSelectable[ival];
    this.dyda.setChanged(ImplAccess.chgColorBack | ImplAccess.chgColorLine);
  ...
  @Override public void setValue(Object[] values){
    if(values.length >=2){
      ...
      if(val1 >= this.colorBorderSelectable.length || val1 <0){ val1 = this.colorBorderSelectable.length -1; } //faulty index, use faulty showing color.
      if(val2 >= this.colorInnerSelectable.length || val2 <0){ val2 = this.colorInnerSelectable.length -1; }
      this.dyda.lineColor = this.colorBorderSelectable[val1];
      this.dyda.backColor = this.colorInnerSelectable[val2];
      this.dyda.setChanged(ImplAccess.chgColorBack | ImplAccess.chgColorLine);
  ...
  implements GralWidgImplAccess_ifc
  {
    ...
    protected GralWidget.DynamicData xxxdyda(){ return GralLed.this.dyda; }
  }
